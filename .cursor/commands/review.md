...
This repo (ohah/gif2mp4) uses the **ohah** GitHub account for posting reviews.
...
4. **Write the AI review (summary + line suggestions)**
   ...
   - **Line-level suggestions**: For each place that needs a change, prepare an inline comment with:
     - **path**: Repo-root-relative path (e.g. `packages/core/src/gif-to-mp4.ts`)
         ...
         "comments": [
           {
             ...
             "line": 42,
             "side": "RIGHT",
             "body": "Prefer clearing the retry timeout.\n\n```suggestion\n  clearTimeout(id);\n```"
           }
         ]
       }
       ````
...
- Run from the repo root with `gh` authenticated. This repo (ohah/gif2mp4): use ohah for posting reviews; switch gh before submit and restore after (see "gh account for this repo" and step 1, step 6).
